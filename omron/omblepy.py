...
def appendCsv(allRecords):
    for userIdx in range(len(allRecords)):
        oldCsvFile = pathlib.Path(f"../user/omron_user{userIdx+1}.csv")
        datesOfNewRecords = [record["datetime"] for record in allRecords[userIdx]]
        if(oldCsvFile.is_file()):
            records = readCsv(f"../user/omron_user{userIdx+1}.csv")
        ...
        with open(f"../user/omron_user{userIdx+1}.csv", mode='w', newline='', encoding='utf-8') as outfile:
