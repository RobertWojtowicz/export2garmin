...
while [ $loop_count -eq 0 ] || [ $i -lt $loop_count ] ; do
	...
	if [ $switch_miscale == "on" ] ; then
		...
		if [ ! -f $path/user/miscale_backup.csv ] ; then
			...
			else echo "$header" > $path/user/miscale_backup.csv
			fi
		...
		if [ $switch_mqtt == "on" ] ; then
  			export $(grep miscale_mqtt_ $path/user/export2garmin.cfg)
		...
		if [ ! -z $miscale_unixtime ] ; then
  			export $(grep miscale_offset $path/user/export2garmin.cfg)
			...
			if grep -q $miscale_cut $path/user/miscale_backup.csv ; then
				miscale_time_tag=`grep -m 1 $miscale_cut $path/user/miscale_backup.csv | awk -F ";" '{print $2}'`
				...
				if (( $miscale_absolute_dif < 30 )) ; then
					...
					if (( $miscale_absolute_shift > 1200 )) ; then
						...
						sed -i "/$miscale_offset_unixtime/d" $path/user/miscale_backup.csv
					else echo "$(timenow) MISCALE * Saving import $miscale_offset_unixtime to miscale_backup.csv file"
						echo $miscale_offset >> $path/user/miscale_backup.csv
					fi
				fi
			else miscale_absolute_shift=`echo ${miscale_time_shift#-}`
				if (( $miscale_absolute_shift > 1200 )) ; then
		...
		if [ -z $miscale_import ] ; then
			...
			if grep -q "MISCALE \* There" /dev/shm/temp.log ; then
				...
				sed -i "/$miscale_import/d" $path/user/miscale_backup.csv
				...
				sed -i "s/to_import;$miscale_import/failed;$miscale_import/" $path/user/miscale_backup.csv
				...
				sed -i "s/failed;$miscale_import_data/uploaded;$miscale_import;$miscale_calc_data;$miscale_time_shift/; s/to_import;$miscale_import_data/uploaded;$miscale_import;$miscale_calc_data;$miscale_time_shift/" $path/user/miscale_backup.csv
				miscale_import_diff=`echo $miscale_calc_data | awk -F ";" '{print $1 ";" $2 ";" $3}'`
				miscale_check_line=`wc -l < $path/user/miscale_backup.csv`
				if [ $miscale_check_line == "2" ] ; then
					sed -i "s/$miscale_import;$miscale_import_diff/$miscale_import;$miscale_import_diff;0.0/" $path/user/miscale_backup.csv
				else miscale_email_user=`echo $miscale_calc_data | awk -F ";" '{print $18}'`
					miscale_weight_last=`grep $miscale_email_user $path/user/miscale_backup.csv | sed -n 'x;$p' | awk -F ";" '{print $5}'`
					...
					sed -i "s/$miscale_import;$miscale_import_diff/$miscale_import;$miscale_import_diff;$miscale_weight_diff/; s/;0;/;0.0;/" $path/user/miscale_backup.csv
				fi
			fi
		fi
